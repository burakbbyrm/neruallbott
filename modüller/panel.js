...
module.exports = (client) => {
  ...
  const bilgiler = {
    oauthSecret: "SPqX7EMRM7lK4I0L8-f2gzd6B8n3RhYH",
    callbackURL: `https://nerualpanel.cf/callback`,
    domain: `https://nerualpanel.cf/`
  };
...
/////
  ...
  const guild1 = client.guilds.get(req.params.guildID);
        const sunucu1 = client.guilds.get(req.params.guildID);

 if (!guild1) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
...
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
  ...
  // DESTEK SİTEM
      ...
      app.post("/dashboard/:guildID/sahtehesap", girisGerekli, async(req, res) => {
    const guild = client.guilds.get(req.params.guildID);
      const sunucu = client.guilds.get(req.params.sunucuID);
   if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
    const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has("MANAGE_GUILD") : false;
  if (!isManaged && !req.session.isAdmin) return res.json({"hata":"Bu sunucuda Sunucuyu Yönet iznin bulunmuyor. Bu yüzden bu sayfaya erişim sağlayamazsın."});
    ...
    res.redirect("/dashboard/"+req.params.guildID+"/sahtehesap");
  });
  
  
  
      app.get("/dashboard/:guildID/sahtehesap", girisGerekli, (req, res) => {
    const guild = client.guilds.get(req.params.guildID);
    const sunucu = client.guilds.get(req.params.guildID);
if (!guild) return res.json({"hata":"Bot "+req.params.sunucuID+" ID adresine sahip bir sunucuda bulunmuyor."});
    const isManaged = guild && !!guild.member(req.user.id) ? guild.member(req.user.id).permissions.has("MANAGE_GUILD") : false;
  ...
  app.get("/dashboard/:sunucuID/sahtehesap/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`fake_${req.params.sunucuID}`) === false) return res.json({"hata": "Destek kanalı "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`fake_${req.params.sunucuID}`)
    res.redirect(`/dashboard/${req.params.sunucuID}/sahtehesap`);
  });
  
   app.get("/dashboard/:sunucuID/sahtehesap/sifirla", girisGerekli, (req, res) => {
    if (client.ayar.has(`fake_${req.params.sunucuID}`) === false) return res.json({"hata": "Destek Rolü "+client.guilds.get(req.params.sunucuID).name+" adlı sunucuda ayarlı olmadığı için sıfırlanamaz."});
    client.ayar.delete(`fake_${req.params.sunucuID}`)
    res.redirect(`/dashboard/${req.params.sunucuID}/sahtehesap`);
  });
